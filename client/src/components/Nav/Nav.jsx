  ...
  return (
    <nav className="flex fixed justify-between bg-gray text-white w-screen">
      ...
      <div className='px-5 xl:px-12 py-3 flex w-full items-center'>
        ...
        <ul className= {`md:flex md:items-center px-6 mx-auto font-poppins font-heading z-[-1] md:z-auto md:static absolute left-0 w-full md:w-auto md:py-0 py-10 md:pl-0 pl-7 md:opacity-100  top[-400px] transition-all ease-in duration-500 ${
          open ? ' bg-gray top-1 opacity-100' : ' top-[-490px]' } md:opacity-100 opacity-0`} >
        ...
        {isAuthenticated ? (
            ...
            <FaUserCircle onClick={() => loginWithRedirect ()} className="w-8 h-8 cursor-pointer" />
          )}   
        </div>        
        </div>
        ...
        <Link onClick={handleClick} className='navbar-burger self-center mr-12 xl:hidden flex items-center'>
        
        {open ? <MdClose className='w-6 h-6 hover:text-green-neon' /> : <MdMenu className='w-6 h-6 hover:text-green-neon' />}
